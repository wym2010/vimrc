...
local function Pcol(above)
  ...
  vim.cmd('normal!' .. (above == 1 and 'P' or 'p'))
  vim.cmd('call cursor(".", ' .. col .. ')')
end

local function QuitWithQuickfixCheck()
...
vim.keymap.set('n', '<silent>ty', ':Translate<CR>')
vim.keymap.set('v', '<silent>ty', ':Translate<CR>')
vim.keymap.set('n', '<silent>tr', '<Plug>TranslateR')
vim.keymap.set('v', '<silent>tr', '<Plug>TranslateRV')
...
vim.api.nvim_set_keymap('i', '<script><silent><nowait><expr> <C-o>', 'codeium#Accept()', { noremap = true, silent = true, expr = true })
...
vim.api.nvim_create_user_command('GMore', function()
  if logenabled == 1 then
    logindex = logindex + 500
    vim.cmd('Gclog! -500 --skip=' .. logindex)
  end
  if logcenabled == 1 then
    logcindex = logcindex + 500
    vim.cmd('Gclog! -500 --skip=' .. logcindex .. ' -- %')
  end
end, {})
...
vim.api.nvim_create_user_command('GLess', function()
  if logenabled == 1 then
    logindex = logindex - 500
    if logindex < 0 then logindex = 0 end
    vim.cmd('Gclog! -500 --skip=' .. logindex)
  end
  if logcenabled == 1 then
    logcindex = logcindex - 500
    if logcindex < 0 then logcindex = 0 end
    vim.cmd('Gclog! -500 --skip=' .. logcindex .. ' -- %')
  end
end, {})
...
vim.keymap.set('n', '<Leader><Leader>', 'gcc<ESC>')
vim.keymap.set('v', '<Leader><Leader>', 'gcc<ESC>')
...
vim.keymap.set('v', '<leader>ac', ':lua vim.lsp.buf.code_action()<CR>')
...
vim.keymap.set('n', '*', function()
  if vim.v.count > 0 then
    ...
    vim.cmd('silent execute "keepjumps normal! *"')
    vim.cmd('call winrestview(' .. vim.fn.string(vim.fn.winsaveview()) .. ')')
    return ''
  end
end, { expr = true })
vim.keymap.set('n', '#', function()
...
vim.g.instant_markdown_browser = 'google-chrome-stable --new-window'
vim.keymap.set('n', '<c-p>', ':silent! InstantMarkdownStop<CR> :InstantMarkdownPreview<CR>')
...
require('lualine').setup({
  ...
  sections = {
    ...
    lualine_x = {
      { gps.get_location, cond = gps.is_available, color = 'WildMenu' },
    },
    lualine_y = {},
  ...
  inactive_sections = {
    ...
    lualine_x = {},
    lualine_x = {
      { gps.get_location, cond = gps.is_available, color = 'Folded' },
    },
    lualine_y = {},
  ...
  tabline = {
    lualine_a = {
      { 'filename', path = 3, shorting_target = 15 },
    },
    lualine_b = {},
    lualine_c = {},
    lualine_x = {},
...
local function my_on_attach(bufnr)
  ...
  vim.keymap.set('n', '<C-e>', api.node.run.system, opts('Run System'))
  vim.keymap.set('n', '<C-k>', api.node.show_info_popup, opts('Info'))
  vim.keymap.set('n', '<C-r>', api.fs.rename_sub, opts('Rename: Omit Path'))
  vim.keymap.set('n', '<C-t>', api.node.navigate.parent_close, opts('Close Directory'))
  ...
  vim.keymap.set('n', 'bd', api.node.navigate.sibling, opts('Previous Sibling'))
  ...
  vim.keymap.set('n', 'm', api.marks.move, opts('Move Bookmark'))
  vim.keymap.set('n', 'n', api.fs.name, opts('Rename'))
  ...
  vim.keymap.set('n', 'S', api.tree.search, opts('Search'))
